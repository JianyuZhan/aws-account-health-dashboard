...
如果未设置 `AWS_HEALTH_DASHBOARD_REGION`，将使用 `AWS_REGION` 作为备选。您也可以在命令行中通过 `--region` 参数直接指定区域。
...
1. 在**每个**组织的管理帐号中，运行 `deploy/management_accounts/create_cross_role.py` 脚本。
   ...
   - 您可以通过环境变量 `AWS_HEALTH_DASHBOARD_REGION` 或使用 `--region` 参数指定区域：
     ```bash
     python create_cross_account_role.py --region us-east-1
     ```
...
1. **安装必要工具**：
   - **安装 AWS CLI**：
     - 请确保已安装 AWS CLI 并配置好凭证。如未安装，请参考[AWS CLI安装文档](https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html)。
   
   - **安装 Node.js 和 npm**：
     - CDK 需要 Node.js 环境。使用以下命令安装：
       ```bash
...
6. **引导 CDK 环境**：
   - 第一次使用 CDK 部署到指定区域时，需要先引导 CDK 环境：
     ...
     cdk bootstrap
     ```
     如果已设置 AWS 凭证，CDK 将自动使用当前账户和区域。
     
     您也可以通过 `--region` 参数指定部署区域：
     ```bash
